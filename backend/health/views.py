...
class HealthRecordViewSet(viewsets.ModelViewSet):
    ...
    def file(self, request, pk=None):
        ...
        if not record.file:
            return Response({"detail": "No file attached to this record"}, status=status.HTTP_404_NOT_FOUND)
        
        try:
            response = FileResponse(record.file.open(), content_type='application/octet-stream')
            response['Content-Disposition'] = f'inline; filename="{record.file.name}"'
            return response
